                 ...
                 <th>Kode Dosen</th>   
                   ...
                   <tbody>    
                        <?php     //query ke database SELECT tabel mahasiswa urut berdasarkan id yang paling besar   
                          $sql = mysqli_query($koneksi, "SELECT a.nim,a.nama,
                          b.kd_mk,b.kd_dosen,b.uts,c.nama_mk,c.sks 
                          FROM mahasiswa AS a 
                          JOIN nilai AS b 
                          ON a.nim=b.nim 
                          JOIN matakuliah AS c 
                          ON c.kd_mk=b.kd_mk") or die(mysqli_error($koneksi));     //jika query diatas menghasilkan nilai > 0 maka menjalankan script di bawah if...   
                                         ...
                                         <td>'.$data['nim'].'</td>      
                                           <td>'.$data['nama'].'</td>    
                                               <td>'.$data['kd_mk'].'</td>      
                                                 ...
                                                 <td>'.$data['nama_mk'].'</td>
                                                 <td>'.$sks['sks'].'</td>  
                                                      <td>         <a href="edit.php?id='.$data['id'].'" class="badge badge-warning">Edit</a>      
                                                           <a href="delete.php?id='.$data['id'].'" class="badge badge-danger" onclick="return confirm(\'Yakin ingin menghapus data ini?\')">Delete</a>        </td>     
                                                          </tr>    
                                                             ';    
